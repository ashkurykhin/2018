...
Short equals long plus the effect of omitted times times the regression of omitted on included 

```{r}
omit_reg <- lm(open~storetype, rossmann)
stargazer(omit_reg, type='text')
```
...
```{r}
rossmann_open <- rossmann %>% filter(open==1)
salereg_only_open <- lm(salest ~ storetype, rossmann %>% filter(open==1))
stargazer(salereg_only_open, type='text')
```
...
```{r}
salesreg_comp <- lm(salest ~ competition, rossmann)
salesreg_comp_open <- lm(salest ~ competition, rossmann %>% filter(open==1))
...
## 9.
```{r}
salesreg_dist <- lm(salest ~ compdistkm, rossmann)

stargazer(salesreg_dist, type = 'text',
          keep.stat = c('n', 'rsq'))
```
...
```{r}
ggplot(rossmann, mapping = aes(x = month(date), y = salest)) + 
  stat_summary(fun.y='mean', geom='path')+
  labs(y = 'Average sales in thousands', x = 'Month')
```
